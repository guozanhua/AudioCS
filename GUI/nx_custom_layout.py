...
def nx_custom_layout(graph):
    ...
    if n == 1:
        ...
        pos = np.asarray([[0.30, 0.70], [0.70, 0.30]], dtype=np.float32)
    elif n == 3:
        pos = np.asarray([[0.50, 0.70], [0.30, 0.30],
                          [0.70, 0.30]], dtype=np.float32)
    elif n == 4:
        pos = np.asarray([[0.30, 0.30], [0.30, 0.70],
                          [0.70, 0.30], [0.70, 0.70]], dtype=np.float32)
